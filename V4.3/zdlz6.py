...
def main(is_head):
    ...
    try:
        ...
        try:
            #已领
            infor = WebDriverWait(driver, 5, 0.1).until(lambda x: x.find_element_by_xpath("//*[@class='layui-layer-content']")).text
            all_infor += [url, infor]
            is_success = True
        except:
            try: 
                alert = WebDriverWait(driver, 20, 0.1).until(lambda x: x.switch_to_alert())
                #获取警告对话框的内容
                infor = alert.text
