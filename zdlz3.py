        ...
        try:
            ...
            for cs in range(max_cs):
                ...
                try: 
                    ...
                    try: 
                        alert = WebDriverWait(driver, 10, 0.1).until(lambda x: x.switch_to_alert())
                        #获取警告对话框的内容
                        infor = alert.text
            ...
            #已领或未知错误
            infor = WebDriverWait(driver, 3, 0.3).until(lambda x: x.find_element_by_xpath('//*[@class="layui-layer-content"]')).text
            all_infor += [url, infor]
