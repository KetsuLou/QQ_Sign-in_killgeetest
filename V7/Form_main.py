...
#-*- coding:utf-8 -*-
#导入窗口
import Form_inf, Form_sys, Form_attention, Form_tools, Form_webset

#导入自动程序
import zdlz1, zdlz2, zdlz3, zdlz4, zdlz5, zdlz6
import zdqd1, zdqd2, sgsqd
...
class Application_ui(Frame):
    ...
    def createWidgets(self):
        ...
        try:
            #尝试创建选择配置
            self.config.add_section('choose')
            #self.Check_zdlz1Var = StringVar(value='1')
            self.config.set('choose', 'zdlz1', '1')
            self.config.set('choose', 'zdlz2', '1')
            self.config.set('choose', 'zdlz3', '1')
            self.config.set('choose', 'zdlz4', '1')
            self.config.set('choose', 'zdlz5', '1')
            self.config.set('choose', 'zdlz6', '1')
            self.config.set('choose', 'zdqd1', '1')
            self.config.set('choose', 'zdqd2', '1')
            self.config.set('choose', 'sgsqd', '1')
...
#-----------------------------------------------------------------------------
        ...
        self.Check_zdlz1Var = StringVar(value=self.config['choose']['zdlz1'])
        ...
        self.Check_zdlz2Var = StringVar(value=self.config['choose']['zdlz2'])
        ...
        self.Check_zdlz3Var = StringVar(value=self.config['choose']['zdlz3'])
        ...
        self.Check_zdlz4Var = StringVar(value=self.config['choose']['zdlz4'])
        ...
        self.Check_zdlz5Var = StringVar(value=self.config['choose']['zdlz5'])
        ...
        self.Check_zdlz6Var = StringVar(value=self.config['choose']['zdlz6'])
...
#-----------------------------------------------------------------------------
        ...
        self.Check_zdqd1Var = StringVar(value=self.config['choose']['zdqd1'])
        ...
        self.Check_zdqd2Var = StringVar(value=self.config['choose']['zdqd2'])
        ...
        self.Check_sgsqdVar = StringVar(value=self.config['choose']['sgsqd'])
...
#-----------------------------------------------------------------------------
        ...
        self.Button_run.place(relx=0.035, rely=0.481, relwidth=0.174, relheight=0.089)
        ...
        self.Button_reset.place(relx=0.208, rely=0.481, relwidth=0.174, relheight=0.089)

        self.style.configure('Button_savechoose.TButton',font=('宋体',9))
        self.Button_savechoose = Button(self.top, text='保存选择', command=self.Button_savechoose_Cmd, style='Button_savechoose.TButton')
        self.Button_savechoose.place(relx=0.035, rely=0.57, relwidth=0.174, relheight=0.089)

        self.style.configure('Button_test.TButton',font=('宋体',9))
        self.Button_test = Button(self.top, text='调试模式', command=self.Button_test_Cmd, style='Button_test.TButton')
        self.Button_test.place(relx=0.208, rely=0.57, relwidth=0.174, relheight=0.089)
#-----------------------------------------------------------------------------
        ...
        self.Button_inf.place(relx=0.618, rely=0.481, relwidth=0.174, relheight=0.089)
        ...
        self.Button_sys.place(relx=0.792, rely=0.481, relwidth=0.174, relheight=0.089)
        ...
        self.Button_tools.place(relx=0.618, rely=0.57, relwidth=0.174, relheight=0.089)
        ...
        self.Button_attention.place(relx=0.792, rely=0.57, relwidth=0.174, relheight=0.089)
#-----------------------------------------------------------------------------
        ...
        self.Check_is_head.place(relx=0.035, rely=0.66, relwidth=0.278, relheight=0.071)
        ...
        self.List_inf.place(relx=0.035, rely=0.802, relwidth=0.938, relheight=0.171)
#-----------------------------------------------------------------------------
class Application(Application_ui):
    ...
    def Button_savechoose_Cmd(self, event=None):
        self.config.read(path_default_sys)
        self.path = self.config['path']['path_ini']
        self.config.set('choose', 'zdlz1', self.Check_zdlz1Var.get())
        self.config.set('choose', 'zdlz2', self.Check_zdlz2Var.get())
        self.config.set('choose', 'zdlz3', self.Check_zdlz3Var.get())
        self.config.set('choose', 'zdlz4', self.Check_zdlz4Var.get())
        self.config.set('choose', 'zdlz5', self.Check_zdlz5Var.get())
        self.config.set('choose', 'zdlz6', self.Check_zdlz6Var.get())
        self.config.set('choose', 'zdqd1', self.Check_zdqd1Var.get())
        self.config.set('choose', 'zdqd2', self.Check_zdqd2Var.get())
        self.config.set('choose', 'sgsqd', self.Check_sgsqdVar.get())
        self.config.write(open(path_default_sys, 'w')) 
        self.List_inf.delete(0, END)
        self.List_inf.insert(END, '已成功保存选择配置！')
    ...
    def Button_run_Cmd(self, event=None):
        ...
        if self.Button_test['text'] == '调试模式': is_test = 0
        else: is_test = 1
        if self.select():
            try:
                if self.Check_zdlz1Var.get() == '1': 
                    ...
                    infor_zdlz1, is_success_zdlz1 = zdlz1.main(max_cs, is_test, is_head, self.offset_)
                    if is_success_zdlz1: self.Check_zdlz1Var.set(0)
                    for infor in infor_zdlz1:
                        self.List_inf.insert(END, infor)

                if self.Check_zdlz2Var.get() == '1': 
                    self.List_inf.insert(END, "自动领赞2运行中，账号：" + self.config['path_lz']['username'])
                    if self.config['path_lz']['username'] == '' or self.config['path_lz']['password'] == '': raise
                    infor_zdlz2, is_success_zdlz2 = zdlz2.main(is_test, is_head)
                    if is_success_zdlz2: self.Check_zdlz2Var.set(0)
                    for infor in infor_zdlz2:
                        self.List_inf.insert(END, infor) 

                if self.Check_zdlz3Var.get() == '1': 
                    self.List_inf.insert(END, "自动领赞3运行中，账号：" + self.config['QQ_path']['QQ'])
                    if self.config['QQ_path']['QQ'] == '': raise
                    infor_zdlz3, is_success_zdlz3 = zdlz3.main(max_cs, is_test, is_head, self.offset_)
                    if is_success_zdlz3: self.Check_zdlz3Var.set(0)
                    for infor in infor_zdlz3:
                        self.List_inf.insert(END, infor)     

                if self.Check_zdlz4Var.get() == '1': 
                    self.List_inf.insert(END, "自动领赞4运行中，账号：" + self.config['QQ_path']['QQ'])
                    if self.config['QQ_path']['QQ'] == '': raise
                    infor_zdlz4, is_success_zdlz4 = zdlz4.main(max_cs, is_test, is_head, self.offset_)
                    if is_success_zdlz4: self.Check_zdlz4Var.set(0)
                    for infor in infor_zdlz4:
                        self.List_inf.insert(END, infor)  

                if self.Check_zdlz5Var.get() == '1': 
                    self.List_inf.insert(END, "自动领赞5运行中，账号：" + self.config['QQ_path']['QQ'])
                    if self.config['QQ_path']['QQ'] == '': raise
                    infor_zdlz5, is_success_zdlz5 = zdlz5.main(max_cs, is_test, is_head, self.offset_)
                    if is_success_zdlz5: self.Check_zdlz5Var.set(0)
                    for infor in infor_zdlz5:
                        self.List_inf.insert(END, infor)  

                if self.Check_zdlz6Var.get() == '1': 
                    self.List_inf.insert(END, "自动领赞6运行中，账号：" + self.config['QQ_path']['QQ'])
                    if self.config['QQ_path']['QQ'] == '': raise
                    infor_zdlz6, is_success_zdlz6 = zdlz6.main(is_test, is_head)
                    if is_success_zdlz6: self.Check_zdlz6Var.set(0)
                    for infor in infor_zdlz6:
                        self.List_inf.insert(END, infor)   
                                   
                if self.Check_zdqd1Var.get() == '1': 
                    self.List_inf.insert(END, "自动签到1运行中，账号：" + self.config['path_lz']['username'])
                    if self.config['path_lz']['username'] == '' or self.config['path_lz']['password'] == '': raise
                    infor_zdqd1, is_success_zdqd1 = zdqd1.main(max_cs, is_test, is_head, self.offset_)
                    if is_success_zdqd1: self.Check_zdqd1Var.set(0)
                    for infor in infor_zdqd1:
                        self.List_inf.insert(END, infor) 
                
                if self.Check_zdqd2Var.get() == '1': 
                    self.List_inf.insert(END, "自动签到2运行中，账号：" + self.config['path_lz']['username'])
                    if self.config['path_lz']['username'] == '' or self.config['path_lz']['password'] == '': raise
                    infor_zdqd2, is_success_zdqd2 = zdqd2.main(is_test, is_head)
                ...
                if self.Check_sgsqdVar.get() == '1': 
                    ...
                    infor_sgsqd, is_success_sgsqd = sgsqd.main(is_test, is_head)
    ...
    def Button_test_Cmd(self, event=None):
        ...
        if self.Button_test['text'] == '调试模式':
            ...
            self.Check_zdlz3Var.set(0)
            self.Check_zdlz4Var.set(0)
            self.Check_zdlz5Var.set(0)
            self.Check_zdlz6Var.set(0)
            self.Check_zdqd1Var.set(0)
            self.Check_zdqd2Var.set(0)
            self.Check_sgsqdVar.set(0)
            self.Check_is_headVar.set(0)
            
            self.Button_test['text'] = '退出调试'
        else:
            self.Check_zdlz1Var.set(self.config['choose']['zdlz1'])
            self.Check_zdlz2Var.set(self.config['choose']['zdlz2'])
            self.Check_zdlz3Var.set(self.config['choose']['zdlz3'])
            self.Check_zdlz4Var.set(self.config['choose']['zdlz4'])
            self.Check_zdlz5Var.set(self.config['choose']['zdlz5'])
            self.Check_zdlz6Var.set(self.config['choose']['zdlz6'])
            self.Check_zdqd1Var.set(self.config['choose']['zdqd1'])
            self.Check_zdqd2Var.set(self.config['choose']['zdqd2'])
            self.Check_sgsqdVar.set(self.config['choose']['sgsqd'])
            self.Check_is_headVar.set('1')
            
            self.Button_test['text'] = '调试模式'
